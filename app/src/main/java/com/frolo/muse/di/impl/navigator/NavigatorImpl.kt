...
import com.frolo.muse.ui.main.editor.song.SongEditorFragment
...
class NavigatorImpl<T> constructor(
    ...
    override fun editSong(song: Song) {
        val fragment = SongEditorFragment.newInstance(song)
        root.pushDialog(fragment)
    }
    ...
    override fun createPlaylist() {
        val fragment = CreatePlaylistDialog.newInstance()
        root.pushDialog(fragment)
    }

    override fun createPlaylist(songs: ArrayList<Song>) {
        val fragment = CreatePlaylistDialog.newInstance(songs)
        root.pushDialog(fragment)
    }
